...
//			response.sendRedirect( projectName+"/"+"blog/2" );
    ...
    public String registerpost(HttpServletRequest request,Model model,String registername,String registerpwd){ 
		...
		Boolean res = User.addUser(user,false);
		if(res) {//false表示注册的时候都没有加头像
			//注册成功
	    	//转到登录 
			...
			//再次进入登陆界面 
	          ...
	          return "forward:/account/register";
		}
    }
    //个人信息
	...
	@RequestMapping(value="/editportait/{userid}", method = RequestMethod.POST)
...
//			return new ModelAndView("error");
			...
			for(MultipartFile item : files)
			{
				if(item.getSize() > 0)
				{
					...
					File file = new File(user.getAbsolutelyHeadPortraitPath());
					try {
